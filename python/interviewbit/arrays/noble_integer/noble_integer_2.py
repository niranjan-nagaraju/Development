...
'''
Solution Outline: O(n+max-min) time, O(max-min) memory
	1. Calculate min, max of A[]
	2. Setup a lookup table, table[min.. max], 
	    or [0..h], h = max-min+1, mapping a[i] -> table[a[i]-min]
	3. Scan A[i] and update frequency of A[i] in table
	4. Scan all non-empty slots in table (non-empty slots, s,  in table => (min+s) is part of A[])
