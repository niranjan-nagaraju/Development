...
Sample run:
	...
	Memory-efficient DP: Avoid using DP table, P
	and just accumulate each txn's profits.
'''
class Solution:
	def max_profits_any_txns(self, A):
		...
		for i in xrange(1, len(A)):
			if A[i] > A[i-1]:
				# If Day i's price > previous day's
				# book profit, and buy back at day i's price.
				max_profits += A[i]-buy
