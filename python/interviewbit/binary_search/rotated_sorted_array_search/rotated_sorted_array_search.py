...
Solution Outline:
	...
	2. If the subarray, A[l:h] is still rotated, A[l] > A[h]
	    A[l:h] will not contain key if key is between A[h] and A[l]
	...
	def rotated_binary_search(self, A, B):
		...
		while l <= h:
			...
			if A[l] > A[mid]:
				...
				# if B is between A[mid] and A[l]
				# search in the other half
				if A[mid] < B < A[l]:
					l = mid+1
				else:
					# B is in the rotated subarray
					h = mid-1
			else: # A[mid] > A[h]
				# A[mid:h] is rotated

				# If B is between A[h] and A[mid]
				# search for B in the other half
				if A[h] < B < A[mid]:
					h = mid-1
				else:
					# Look for B within the rotated subarray
					l = mid+1
