...
'''
Solution Outline:
	Brute-force (exhaustive-search, O(2ᵐⁿ))
...
class GridWithRewards(object):
	...
	def find_max_rewards(self):
		def max_rewards(i, j):
			if (j == 0):
				# First column
				return sum([self.grid[x][0] for x in xrange(self.m-1, i-1, -1)]), [(x,0) for x in xrange(self.m-1, i-1, -1)]
			
			if (i == self.m-1):
				# Last row
				return sum([self.grid[self.m-1][x] for x in xrange(0, j+1)]), [(self.m-1, x) for x in xrange(0, j+1)]

			left_rewards, lpath = max_rewards(i, j-1)
			down_rewards, dpath = max_rewards(i+1, j)

			return (self.grid[i][j] + max(left_rewards, down_rewards)), \
