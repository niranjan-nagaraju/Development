...
'''
Solution Outline: O(n) time, O(1) memory version
...
4. NOTE: Since we only need to look behind two elements, we don't need an entire DP table [0..x]
   for some index x, but rather just a pair storing (DP[x-2], DP[x-1]) bringing the memory to O(1).
...
def max_non_adjacent_subset_sum(arr):
	...
	for i in xrange(2, len(arr)):
		...
		if arr[i] + look_behind[0] > max_sum:
			max_sum = arr[i] + look_behind[0]

		look_behind[0] = look_behind[1]
		look_behind[1] = max_sum
