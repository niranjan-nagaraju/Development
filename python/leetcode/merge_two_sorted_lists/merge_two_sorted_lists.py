...
class Solution(object):
	def mergeTwoLists(self, l1, l2):
		...
		# Append 'node' to the end of l3
		def append_to_l3(node):
			if not l3_head:
				return (node, node)
		...
		# Remove head node from linked list, l
		# and return the node 'sanitized'
		def remove_head(l):
			node = l
			l = l.next
			node.next = None
		...
		while l1 and l2:
			if l1.val < l2.val:
				l1, node = remove_head(l1)
			else:
				l2, node = remove_head(l2)

			l3_head, l3_tail = append_to_l3(node)
		...
		if l1:
			...
			l3_head, l3_tail = append_to_l3(l1)

		if l2:
			# l3_tail wont be updated to the real tail
			# but this is OKAY
			# we just need to return the head of l3
			l3_head, l3_tail = append_to_l3(l2)
