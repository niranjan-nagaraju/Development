...
Algorithm outline (BFS-based):
	...
	NOTE: BFS-based path discovery gives us the shortest path from a to b
	      Also helps reduce precision being lost across multiple levels in DFS which might not always return the shortest path.
'''
...
class Solution(object):
	def calcEquation(self, equations, values, queries):
		...
		# BFS-based path utility that accumulates products in the current path
		# a/b=k, b/c=l, a/c = a/b*b/c = k*l
		def bfs_path_util(start, end):
			visited = defaultdict(bool)
			bfs_q = [(start, 1.0)]
			visited[start] = True
			
			while bfs_q:
				curr, value = bfs_q.pop(0)
				if curr == end:
					return round(value,5)

				for b,v in graph[curr]:
					if not visited[b]:
						bfs_q.append((b, value*v))
						visited[b] = True

			# BFS couldn't find a path from a->b
		...
		# Use BFS to find a path that calculates a/b
		def calculate_query_and_add_to_answer(a, b):
			...
			answers.append(bfs_path_util(a, b))
