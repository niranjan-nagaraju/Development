...
{
	public:
		std::vector<int> count_distinct_numbers_in_window(std::vector<int> &A, int k) {
			std::vector<int> window_distinct_counts;
			if (k == 0 || k > A.size())
				return window_distinct_counts;

			if (k == 1)
				return std::vector<int>(A.size(), 1);
			 ...
			 */
			num_distinct = frequencies.size();
			window_distinct_counts.push_back(num_distinct);

			for(int i=k; i<A.size(); i++) {
				int prev_sow = A[i-k];
				int new_eow = A[i];

				frequencies[prev_sow]--;
...
main(void)
{
...

	{
		...
		std::vector<int> expected = {2,3,3,2};
		assert(s.count_distinct_numbers_in_window(A, 3) == expected);
	}

	{
		std::vector<int> A = {1,2,1,3,4,3};
		std::vector<int> expected = {2,2,2,2,2};
		assert(s.count_distinct_numbers_in_window(A, 2) == expected);
	}

	{
		std::vector<int> A = {1,2,1,3,4,3};
		std::vector<int> expected = {1,1,1,1,1,1};
		assert(s.count_distinct_numbers_in_window(A, 1) == expected);
	}

	{
		std::vector<int> A = {1,1,2,3,3,4};
		std::vector<int> expected = {1,2,2,1,2};
		assert(s.count_distinct_numbers_in_window(A, 2) == expected);
	}
