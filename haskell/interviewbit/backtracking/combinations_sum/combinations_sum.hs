...
combinations_sum' [] _ _ = []
combinations_sum' c t prefix
	| (t < 0) = []
	| (t == 0) = [prefix]
	| otherwise = combinations_sum' c (t - head c) (prefix ++ [head c]) ++
...
main = do
	putStr $ assert ( (combinations_sum [8,10,6,11,16,8] 28) == 
		[[6, 6, 6, 10], [6, 6, 8, 8], [6, 6, 16], [6, 11, 11], [8, 10, 10]] ) ""
	putStr $ assert ( (combinations_sum [4,3,2,7] 7) == [ [2,2,3], [3,4], [7] ] ) ""
	putStr $ assert ( (combinations_sum [2,3,6,7] 7) == [ [2,2,3], [7] ] ) ""
