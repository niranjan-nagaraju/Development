...
poll_timer (void *t)
{
	threaded_timer_t *timer = (threaded_timer_t *)t;
	int currtime;
...
{
	threaded_timer_t *timer = (threaded_timer_t *) malloc(sizeof(threaded_timer_t));
...
int
reset_timer(threaded_timer_t *timer, int interval, unsigned int recurrent, unsigned int max_times, unsigned int poll_interval, callback_t callback, void *args)
{
	int currtime = time(NULL);
...
void 
delete_timer(threaded_timer_t *timer)
{
	if (timer) {
		pthread_mutex_lock(&(timer->mutex));
		free(timer);
...
void 
start_timer(threaded_timer_t *timer)
{
