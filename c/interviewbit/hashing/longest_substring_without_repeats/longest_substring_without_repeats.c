...
int find_longest_distinct_substring(char *A)
{
	int index_lookup[NUM_CHARS];
	int sow=0, eow=0;
	...
	index_lookup[A[0]] = 0;
	for (i=1; A[i]; i++) {
		int idx = index_lookup[A[i]];
		if (idx != -1) {
			 /* Found A[i] in lookup table and */
			if (idx >= sow) {
				/** 
				 * A[i] is in current window
				 *
...
int main(void)
{
	assert(find_longest_distinct_substring("") == 0);
	assert(find_longest_distinct_substring("A") == 1);
	assert(find_longest_distinct_substring("abcdbacea") == 5);
	assert(find_longest_distinct_substring("abcabcbb") == 3);
	assert(find_longest_distinct_substring("bbbbb") == 1);
	assert(find_longest_distinct_substring("g4L8WKvi") == 8);
